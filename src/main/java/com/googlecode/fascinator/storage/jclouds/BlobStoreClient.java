...
public class BlobStoreClient {
    ...
    private static void init() throws StorageException {
        ...
        provider = systemConfig.getString("swift", "storage", "blobstore",
                "provider");
        credential = systemConfig.getString("", "storage", "blobstore",
                "password");
        identity = systemConfig.getString("", "storage", "blobstore",
                "identity");
        containerName = systemConfig.getString(DEFAULT_CONTAINER_NAME,
                "storage", "blobstore", "containerName");
        location = systemConfig.getString(null, "storage", "blobstore",
                "location");
        fileSystemLocation = systemConfig.getString(null, "storage",
                "blobstore", "fileSystemLocation");
        supportsUserMetadataSetting = systemConfig.getBoolean(null, "storage",
                "blobstore", "supportsUserMetadata");
    ...
    private static BlobStore blobStoreConnect() throws StorageException {
        ...
        if (StringUtils.isNotEmpty(location)) {
            ...
            if (loc == null) {
                throw new StorageException(
                        location + " location not found in Blobstore");
            }
        }
